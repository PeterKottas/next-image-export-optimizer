...
function ExportedImage({ src, priority = false, loading, lazyRoot = null, lazyBoundary = "200px", className, width, height, objectFit, objectPosition, onLoadingComplete, unoptimized, placeholder = "blur", blurDataURL, onError, ...rest }) {
    ...
    return (React.createElement(Image, { ...rest, ...(typeof src === "object" && src.width && { width: src.width }), ...(typeof src === "object" && src.height && { height: src.height }), ...(width && { width }), ...(height && { height }), ...(loading && { loading }), ...(lazyRoot && { lazyRoot }), ...(lazyBoundary && { lazyBoundary }), ...(className && { className }), ...(objectFit && { objectFit }), ...(objectPosition && { objectPosition }), ...(onLoadingComplete && { onLoadingComplete }), ...(placeholder && { placeholder }), ...(unoptimized && { unoptimized }), ...(priority && { priority }), ...(imageError && { unoptimized: true }), loader: imageError || unoptimized === true
            ? fallbackLoader
